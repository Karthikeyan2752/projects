...
public class WandererControlView {
	
	private WandererControlViewModel wandererControlViewModel;
	private Scanner scanner = new Scanner(System.in);
	
	public WandererControlView() {
		this.wandererControlViewModel = new WandererControlViewModel(this);
	}
	...
	private void printNotifications(Wanderer wanderer) {
		...
		Stack<String> notifications = wandererControlViewModel.getNotifications(wanderer);
	...
	private void getPost(Wanderer wanderer) {
		List<Post> posts = wandererControlViewModel.getPost(wanderer);
		if (posts.isEmpty()) {
	...
	private void viewFollowings(Wanderer wanderer) {
		List<Wanderer> followers = wandererControlViewModel.getFollowers(wanderer);
	...
	private void viewFollowers(Wanderer wanderer) {
		List<Wanderer> followings = wandererControlViewModel.getFollowings(wanderer);
	...
	private void personalisedFeed(Wanderer wanderer) {
		...
		List<Post> posts = wandererControlViewModel.getPersonalisedPost(wanderer);
		for (Post post : posts) {
			System.out.println(post.toString());
			...
			byte choice = UIValidator.getChoice();
			if (choice == 1) {
				String result = wandererControlViewModel.thumbsUp(post, wanderer);
				System.out.println(result);
			} else if (choice == 2) {
				String result = wandererControlViewModel.thumbsDown(post, wanderer);
				System.out.println(result);
			}else if(choice ==3) {
	...
	private void requests(Wanderer wanderer1) {
		List<Wanderer> wanderers = wandererControlViewModel.getRequests(wanderer1);
		...
		for (int i = 0; i < wanderers.size(); i++) {
			...
			if (choice == 1) {
				String result = wandererControlViewModel.acceptRequest(wanderer1, wanderer2);
				System.out.println(result);
			} else if (choice == 2) {
				String result = wandererControlViewModel.acceptRequestAndFollow(wanderer1, wanderer2);
				System.out.println(result);
			}
		}
	...
	private void follow(Wanderer wanderer1) {
		List<Wanderer> wanderers = wandererControlViewModel.getWanderers();
		for (Wanderer wanderer2 : wanderers) {
			...
			int choice = UIValidator.getIntChoice();
			if (choice == 1) {
				String result = wandererControlViewModel.sendRequest(wanderer1, wanderer2);
				System.out.println(result);
			}
		}
		getChoiceAndProceed(wanderer1);
	...
	private void post(Wanderer wanderer) {
		...
		String result = wandererControlViewModel.post(wanderer, postString);
		System.out.println(result);
		getChoiceAndProceed(wanderer);
	}

	private void wander(Wanderer wanderer) {
		List<Post> randomPosts = wandererControlViewModel.getRandomPost(wanderer);
