...
public class CalculatorView implements CalculatorViewCallBack {
	...
	private void ageCalculator() {
		...
		String result = calculatorController.calculateAge(birthDay);
	...
	private void areaCalculator() {
		...
		if(choice==2) {
			...
			area = calculatorController.calculateSquareArea(side);
			System.out.println("area = " + area);
		}else if(choice==1) {
			System.out.println("Enter the radius of circle : \n");
			double radius = scanner.nextDouble();
			area = calculatorController.calculateCircleArea(radius);
			System.out.println("area = " + area);
		}else if(choice==3) {
			...
			area = calculatorController.calculateRectangleArea(length,breadth);
			System.out.println("area = "+area);
		}
		mainPage();
	}
	...
	private void scientificCalculator() {
		...
		double result = calculatorController.scientificCalculator(choice, number);
		System.out.println(result);
		mainPage();
	}
	...
	private void arithmeticCalculator() {
		...
		double result = calculatorController.calculate(equation);
	...
	private void simpleIntrestCalculator() {
		...
		double result = calculatorController.calculateSimpleIntrest(principal, time, rate);
